      ...
      @utils.accumulate_errors("One or more existing release pull requests conflicts with this release") do
        regex =
          if @request_spec.single_component?
            %r{release/component/#{@request_spec.resolved_components.first.component_name}/#{@target_branch}}
          else
            %r{release/multi/\d{14}-\d{6}/#{@target_branch}}
          end
        @repository.find_release_prs(branch: @target_branch).each do |pr|
          if regex.match?(pr.head_ref)
            @utils.error("Pull request #{pr.number} is already open.")
          end
        end
      end
      self
