...
class ReleaseUtils
  ...
  def_delegators :@repo_settings,
                 ...
                 :release_commit_tags
  def_delegators :@repo_settings,
                 :all_unit_names, :unit_settings, :all_unit_settings
  def_delegators :@repo_settings,
                 ...
                 :unit_name_from_release_branch, :release_related_branch?
  ...
  def current_unit_version(unit_name, at: nil)
    unit = releasable_unit(unit_name)
    if unit.type == "gem"
      current_version_from_version_rb(unit, at: at)
    else
      current_version_from_changelog(unit, at: at)
    end
  end

  def current_version_from_version_rb(unit, at: nil)
    content =
      if at
        path = unit.version_rb_path(from: :context)
        capture(["git", "show", "#{at}:#{path}"])
      else
        File.read(unit.version_rb_path(from: :absolute))
      end
    const = unit.version_constant.join("::")
    content += "\nputs ::#{const}\n"
    capture_ruby(content).strip
  end

  def current_version_from_changelog(unit, at: nil)
    content =
      if at
        path = unit.changelog_path(from: :context)
        capture(["git", "show", "#{at}:#{path}"])
      else
        File.read(unit.changelog_path(from: :absolute))
      end
    match = %r{### v([\d.]+) / \d\d\d\d-\d\d-\d\d}.match(content)
    match ? match[1] : "0.0.0"
  end

  def verify_library_version(gem_name, gem_vers)
    logger.info("Verifying #{gem_name} version file ...")
    lib_vers = current_unit_version(gem_name)
  ...
  def load_repo_settings
    ...
    @repo_settings = RepoSettings.new(info)
  ...
  def build_releasable_units
    @releasable_units = {}
    errors = []
    @repo_settings.all_unit_settings.each do |settings|
      releasable = ReleasableUnit.build(settings, tool_context.context_directory)
      errors += releasable.validation_errors(tool_context)
  ...
  def multiple_released_gems_and_versions(pr_info)
    ...
    gems.each_with_object({}) do |gem_name, result|
      result[gem_name] = gem_version = current_unit_version(gem_name, at: merge_sha)
