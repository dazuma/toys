...
module ToysReleaser
  ...
  class Repository
    ...
    #
    def find_release_prs(unit_name: nil, merge_sha: nil, label: nil) # rubocop:disable Metrics/AbcSize
    ...
    #
    def released_units_and_versions(pull)
      single_released_unit_and_version(pull) || multiple_released_units_and_versions(pull)
    end
    ...
    #
    def single_released_unit_and_version(pull_request)
      unit_name =
        if @settings.all_unit_names.size == 1
          @settings.default_unit_name
        else
          unit_name_from_release_branch(pull_request.head_ref)
        end
      ...
      version = unit.current_version(at: pull_request.merge_commit_sha)
