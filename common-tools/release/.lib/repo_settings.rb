...
module ToysReleaser
  ...
  ##
  # Configuration of a single releasable unit
  #
  class UnitSettings
    ##
    # Create a UnitSettings from input data structures
    #
    # @param info [Hash] Nested hash input
    # @param has_multiple_units [boolean] Whether there are other units
    # 
    def initialize(repo_settings, info, has_multiple_units)
      @name = info["name"]
      @directory = info["directory"] || (has_multiple_units ? name : ".")
    ...
    ##
    # @return [String] The type of releasable unit. Default is `"unit"`.
    #     Subclasses may define other types.
    #
    attr_reader :type

    ##
    # @return [String] The directory within the repo in which the releasable
    #     unit is located
    #
    attr_reader :directory

    ##
    # @return [String] Path to the changelog relative to the unit's directory
    #
    attr_reader :changelog_path

    ##
    # @return [String] Path to version.rb relative to the unit's directory
    #
    ...
    ##
    # @return [Array<String>,nil] A tool and params to run prior to each unit
    #     build, or nil if there is no pre-build tool.
    #
    attr_reader :pre_builder_tool

    ##
    # @return [Array<String>,nil] A tool and params to run at the end of each
    #     unit build, or nil if there is no post-build tool.
    #
  ...
  ##
  # Configuration of a ruby gem as a releasable unit
  #
  class GemSettings < UnitSettings
    ##
    ...
    def initialize(repo_settings, info, has_multiple_units)
      super
      @type = "gem"
      @gh_pages_directory = info["gh_pages_directory"] || (has_multiple_units ? name : ".")
      @gh_pages_version_var = info["gh_pages_version_var"] ||
        (has_multiple_units ? "version_#{name}".tr("-", "_") : "version")
      @gh_pages_enabled = info.fetch("gh_pages_enabled",
        repo_settings.gh_pages_enabled || info.key?("gh_pages_directory") || info.key?("gh_pages_version_var"))
  ...
  class RepoSettings
    ...
    ##
    # @return [String] The name of the default unit to release
    #
    attr_reader :default_unit_name

    ##
    # @return [Array<Array<String>>] An array of groups of unit names whose
    ...
    ##
    # @return [Array<String>] A list of all releaseable unit names.
    #
    def all_unit_names
      @units.keys
    end
    ...
    ##
    # @return [Array<UnitSettings>] A list of all releaseable unit settings.
    #
    def all_unit_settings
      @units.values
    end

    ##
    # @param name [String] Name of a releaseable unit.
    # @return [UnitSettings,nil] The releaseable unit settings for the given
    ...
    def read_unit_info(info)
      @units = {}
      @default_unit_name = nil
      @has_multiple_units = (info["units"]&.size.to_i + info["gems"]&.size.to_i) > 1
      info["gems"]&.each do |unit_info|
        read_unit_settings("gems", unit_info)
      end
      info["units"]&.each do |unit_info|
        read_unit_settings("units", unit_info)
      end
      @errors << "No releasable units found" if @units.empty?
    end

    def read_unit_settings(key, unit_info)
      type = unit_info["type"]
      unit =
        if key == "gems" || type == "gem"
          GemSettings.new(self, unit_info, @has_multiple_units)
        else
          UnitSettings.new(self, unit_info, @has_multiple_units)
        end
      if unit.name.empty?
        @errors << "A unit under #{key} is missing a name"
      elsif @units[unit.name]
        @errors << "Duplicate unit #{unit.name.inspect} under #{key}"
      else
    ...
    def read_coordination_info(info)
      if info["coordinate_versions"]
        @coordination_groups = [@units.keys]
        return
      end
      ...
      @coordination_groups.each do |group|
        group.each do |member|
          if !@units.key?(member)
            @errors << "Unrecognized unit #{member.inspect} listed in a coordination group"
          elsif seen.key?(member)
            @errors << "Unit #{member.inspect} is in multiple coordination groups"
          else
            seen[member] = true
          end
        end
