...
module ToysReleaser
  ##
  # Set of tasks that can be used in a pipeline
  #
  module Tasks
    def self.run(type:, name:, options:,
      ...
      task_class = nil
      begin
        task_class = const_get(type)
      rescue ::NameError
        repository.utils.error("Unknown task type: #{type}")
        return
      end
      task = task_class.new(repository: repository, component: component, version: version,
    ...
    class Base
      ...
      def option(key, required: false, default: nil)
        ...
        if !value.nil?
          ...
          exit_task("Missing option: #{key.inspect} for task #{self.class} (name = #{name.inspect})")
        else
          default
        end
      end

      def exit_task(error_message = nil, abort_pipeline: false)
      ...
      def pre_tool
        ...
        unless result.success?
          exit_task("Pre-build tool failed: #{cmd}. Check the logs for details.")
        end
      ...
      def pre_command
        ...
        unless result.success?
          exit_task("Pre-build command failed: #{cmd.inspect}. Check the logs for details.")
        end
      ...
      def check_gh_pages_enabled(required: false)
        if (required || option("require_gh_pages_enabled")) && component_settings.gh_pages_enabled
          utils.log("Skipping step because gh_pages is not enabled.")
          exit_task
        end
      end
    ...
    class Tool < Base
      def run
        ...
        unless result.success?
          exit_task("Tool failed: #{tool.inspect}. Check the logs for details.",
    ...
    class Command < Base
      def run
        ...
        unless result.success?
          exit_task("Command failed: #{command.inspect}. Check the logs for details.",
                    abort_pipeline: option("abort_pipeline_on_error"))
        end
    ...
    class BuildGem < Base
      def run
        ...
        unless result.success?
          exit_task("Gem build failed for #{component.name} #{release_version}. Check the logs for details.")
        end
    ...
    class BuildYard < Base
      def run
        ...
        if !result.success? || !::File.directory?("doc")
          exit_task("Yard build failed for #{component.name} #{release_version}. Check the logs for details.")
        end
    ...
    class ReleaseGem < Base
      ...
      def push_gem
        ...
        unless result.success?
          exit_task("Rubygems push failed for #{component.name} #{release_version}. Check the logs for details.")
        end
    ...
    class PushGhPages < Base
      ...
      def setup_gh_pages_dir
        ...
        exit_task("Unable to access the gh-pages branch.") unless @gh_pages_dir
      ...
      def push_docs_to_git # rubocop:disable Metrics/AbcSize
        ::Dir.chdir(@gh_pages_dir) do
          ...
          if dry_run?
            ...
            unless result.success?
              exit_task("Docs publication failed for #{component.name} #{release_version}. Check the logs for details.")
            end
    ...
    class GitHubRelease < Base
      ...
      def push_tag # rubocop:disable Metrics/AbcSize
        ...
        if dry_run?
          ...
          unless result.success?
            exit_task("Unable to create release #{tag_name}. Check the logs for details.")
          end
