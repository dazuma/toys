...
module ToysReleaser
  ...
  class Performer
    ...
    def internal_perform_release(component, version, result)
      ...
      artifact_dir = ToysReleaser::ArtifactDir.new(@work_dir)
      begin
        component.cd do
          component.settings.tasks.each do |task_settings|
            result_code = ToysReleaser::Tasks.run(
              type: task_settings.task, name: task_settings.name, options: task_settings.options,
