...
module ToysReleaser
  ...
  class Performer
    ...
    ##
    # @return [Hash] the GitHub pull request resource
    ...
    def resolve_ref_and_pr(ref, pr_info)
      ...
      case pr_info
      when ::Integer, ::String
        @pr_info = @repository.load_pr(pr_info.to_i)
        @utils.error("Pull request number #{pr_info} not found.") if @pr_info.nil?
      when ::Hash
        @pr_info = pr_info
      end
    ...
    def tag_already_exists?(unit, version, result)
      ...
      exec_result = @utils.exec(cmd, out: :null, e: false)
