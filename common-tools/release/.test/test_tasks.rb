...
describe ToysReleaser::Tasks do
  ...
  def make_basic_task(type, component: nil, version: nil, dry_run: true, options: {}, performer_result: nil)
    component ||= toys_component
    version ||= sample_release_version
    performer_result ||= make_basic_result(component, version)
    ToysReleaser::Tasks.const_get(type).new(
      repository: repository, component: component, version: version, performer_result: performer_result,
      artifact_dir: artifact_dir, dry_run: dry_run, git_remote: "origin", name: task_name, options: options
    )
  end
  ...
  describe "Base" do
    def make_task(options)
      make_basic_task("Base", options: options)
    end

    it "gets options" do
      task = make_task({"foo" => "bar"})
      assert_equal("bar", task.option("foo"))
    end

    it "Runs a successful command" do
      fake_tool_context.stub_exec(["command_success"], output: "command was run")
      task = make_task({"pre_command" => ["command_success"]})
      task.pre_command
    ...
    it "Runs a failing command" do
      ...
      task = make_task({"pre_command" => ["command_failure"]})
      assert_raises(ToysReleaser::Tasks::TaskExit) do
        task.pre_command
      end
  ...
  describe "Tool" do
    def make_task(tool, abort_pipeline_on_error: false)
      make_basic_task("Tool", options: {"tool" => tool, "abort_pipeline_on_error" => abort_pipeline_on_error})
    end

    it "runs a succeeding tool" do
      fake_tool_context.stub_separate_tool(["sample-tool-success"], output: "This is a sample tool.")
      task = make_task(["sample-tool-success"])
      task.run
    ...
    it "runs a failing tool" do
      ...
      task = make_task(["sample-tool-failure"])
      assert_raises(ToysReleaser::Tasks::TaskExit) do
        task.run
      end
    ...
    it "runs a failing tool and aborts the pipeline" do
      fake_tool_context.stub_separate_tool(["sample-tool-failure"], result_code: 1, output: "Tool run failed.")
      task = make_task(["sample-tool-failure"], abort_pipeline_on_error: true)
      assert_raises(ToysReleaser::Tasks::AbortingExit) do
        task.run
      end
      assert_includes(fake_tool_context.console_output, "Tool run failed.")
      assert_includes(fake_tool_context.console_output,
  ...
  describe "Command" do
    def make_task(command, abort_pipeline_on_error: false)
      make_basic_task("Command", options: {"command" => command, "abort_pipeline_on_error" => abort_pipeline_on_error})
    end

    it "runs a succeeding command" do
      fake_tool_context.stub_exec(["command-success"], output: "This is a sample command.")
      task = make_task(["command-success"])
      task.run
    ...
    it "runs a failing command" do
      ...
      task = make_task(["command-failure"])
      assert_raises(ToysReleaser::Tasks::TaskExit) do
        task.run
      end
      assert_includes(fake_tool_context.console_output, "Command run failed.")
    ...
    it "runs a failing command and aborts the pipeline" do
      fake_tool_context.stub_exec(["command-failure"], result_code: 1, output: "Command run failed.")
      task = make_task(["command-failure"], abort_pipeline_on_error: true)
      assert_raises(ToysReleaser::Tasks::AbortingExit) do
        task.run
      end
      assert_includes(fake_tool_context.console_output, "Command run failed.")
  ...
  describe "BuildGem" do
    def make_task(component)
      make_basic_task("BuildGem", component: component)
    end

    it "builds the toys gem" do
      task = make_task(toys_component)
  ...
  describe "BuildYard" do
    def make_bundle_task(component)
      make_basic_task("Bundle", component: component)
    end

    def make_task(component, pre_tool: nil)
      make_basic_task("BuildYard", component: component, options: {"pre_tool" => pre_tool})
    end
    ...
    it "builds docs for the toys-core gem" do
      bundle_task = make_bundle_task(core_component)
      task = make_task(core_component)
      in_component_directory(core_component) do
    ...
    it "builds docs for the toys gem with a pre-tool" do
      bundle_task = make_bundle_task(toys_component)
      task = make_task(toys_component, pre_tool: ["copy-core-docs"])
      in_component_directory(toys_component) do
        capture_subprocess_io do
          bundle_task.run
        end
  ...
  describe "ReleaseGem" do
    ...
    let(:build_task_name) { "my_build_task" }

    def make_task(dry_run: true)
      make_basic_task("ReleaseGem", component: toys_component, dry_run: dry_run, performer_result: performer_result,
                      options: {"input" => build_task_name})
    end
    ...
    it "does a real release" do
      stub_version_check(false)
      pkg_path = ::File.join(artifact_dir.get(build_task_name), "toys-#{sample_release_version}.gem")
      fake_tool_context.stub_exec(["gem", "push", pkg_path])
      fake_tool_context.prevent_real_exec_prefix(["gem", "push"])
      task = make_task(dry_run: false)
      task.run
      assert_equal(1, performer_result.successes.size)
  ...
  describe "PushGhPages" do
    ...
    def make_task(version, performer_result, dry_run: true)
      make_basic_task("PushGhPages", component: toys_component, dry_run: dry_run, performer_result: performer_result,
                      version: Gem::Version.new(version), options: {"input" => build_task_name})
    end

    def make_dummy_docs
      ::Dir.chdir(artifact_dir.get(build_task_name)) do
        ::FileUtils.mkdir("doc")
    ...
    it "aborts if docs exist" do
      ...
      task = make_task(version, performer_result)
      assert_raises(ToysReleaser::Tasks::TaskExit) do
        capture_subprocess_io do
          task.run
        end
    ...
    it "does a dry run publish" do
      ...
      make_dummy_docs
      performer_result = make_performer_result(version)
      task = make_task(version, performer_result, dry_run: true)
      capture_subprocess_io do
        task.run
      end
      assert_equal(1, performer_result.successes.size)
    ...
    it "does a real publish" do
      ...
      make_dummy_docs
      performer_result = make_performer_result(version)
      task = make_task(version, performer_result, dry_run: false)
      fake_tool_context.stub_exec(["git", "push", task.git_remote, "gh-pages"])
    ...
    it "fails to publish" do
      ...
      performer_result = make_performer_result(version)
      task = make_task(version, performer_result, dry_run: false)
      fake_tool_context.stub_exec(["git", "push", task.git_remote, "gh-pages"], result_code: 1)
      fake_tool_context.prevent_real_exec_prefix(["git", "push"])
      assert_raises(ToysReleaser::Tasks::TaskExit) do
        capture_subprocess_io do
          task.run
  ...
  describe "GitHubRelease" do
    ...
    def make_task(dry_run: true)
      make_basic_task("GitHubRelease", component: toys_component, dry_run: dry_run, performer_result: performer_result)
    end
