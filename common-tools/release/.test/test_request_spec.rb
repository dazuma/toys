...
describe ToysReleaser::RequestSpec do
  ...
  it "resolves default changes to Toys at HEAD" do
    ...
    assert(request_spec.resolved_units.size <= 3)
  end
  ...
  it "resolves default changes to Toys at v0.15.6 tag" do
    ...
    assert_empty(request_spec.resolved_units)
  end
  ...
  it "resolves default changes to Toys just before v0.15.6 tag" do
    ...
    # had no updates.
    resolved_units = request_spec.resolved_units
    assert_equal(2, resolved_units.size)
    assert_equal("toys-core", resolved_units[0].unit_name)
    assert_equal(::Gem::Version.new("0.15.6"), resolved_units[0].version)
    core_changeset = resolved_units[0].change_set
    ...
    assert_equal("toys", resolved_units[1].unit_name)
    assert_equal(::Gem::Version.new("0.15.6"), resolved_units[1].version)
    toys_changeset = resolved_units[1].change_set
  ...
  it "resolves default changes to Toys two commits before v0.15.6 tag" do
    ...
    # At this point, toys had a fix but the other units had no updates.
  ...
  it "resolves requested versioned changes to Toys common-tools two commits before v0.15.6 tag" do
    ...
    request_spec.resolve_versions(repository, release_ref: "4c620495f915fef39d1583170beb6489d0c7073d")
    # We should force a release to common-tools but ignore the nonrequested units
    resolved_units = request_spec.resolved_units
    assert_equal(1, resolved_units.size)
    assert_equal("common-tools", resolved_units[0].unit_name)
    assert_equal(::Gem::Version.new("0.15.5.2"), resolved_units[0].version)
    changeset = resolved_units[0].change_set
  ...
  it "resolves requested semver changes to Toys common-tools two commits before v0.15.6 tag" do
    ...
    assert_equal(1, resolved_units.size)
    assert_equal("common-tools", resolved_units[0].unit_name)
    assert_equal(::Gem::Version.new("0.16.0"), resolved_units[0].version)
    changeset = resolved_units[0].change_set
    assert_equal(1, changeset.change_groups.size)
    assert_equal(1, changeset.change_groups[0].changes.size)
    assert_nil(changeset.change_groups[0].header)
  ...
  it "resolves requested versioned changes to Toys toys-core two commits before v0.15.6 tag" do
    ...
    assert_nil(core_changeset.change_groups[0].header)
    assert_equal("toys", resolved_units[1].unit_name)
    assert_equal(::Gem::Version.new("0.16.0"), resolved_units[1].version)
    toys_changeset = resolved_units[1].change_set
    assert_equal(1, toys_changeset.change_groups.size)
    assert_equal(1, toys_changeset.change_groups[0].changes.size)
    refute_nil(toys_changeset.change_groups[0].header)
