...
describe ToysReleaser::RepoSettings do
  ...
  it "loads the toys repo settings" do
    ...
    assert_equal("toys-core", settings.default_unit_name)
    assert_equal(//, settings.required_checks_regexp)
    assert_equal(1800, settings.required_checks_timeout)
    assert_nil(settings.default_pre_builder_tool)
    assert_nil(settings.default_post_builder_tool)
    assert_nil(settings.default_gem_builder_tool)
    assert_nil(settings.default_docs_builder_tool)
    ...
    assert_equal(["toys-core", "toys", "common-tools"], settings.all_unit_names)
    assert_equal(["toys-core", "toys", "common-tools"], settings.all_unit_settings.map(&:name))
    assert_equal([["toys-core", "toys"]], settings.coordination_groups)

    toys_core_settings = settings.unit_settings("toys-core")
    ...
    assert_equal(["copy-core-docs"], toys_core_settings.pre_builder_tool)
    assert_nil(toys_core_settings.post_builder_tool)
    assert_nil(toys_core_settings.gem_builder_tool)
    assert_nil(toys_core_settings.docs_builder_tool)

    toys_settings = settings.unit_settings("toys")
    ...
    assert_nil(toys_settings.pre_builder_tool)
    assert_nil(toys_settings.post_builder_tool)
    assert_nil(toys_settings.gem_builder_tool)
    assert_nil(toys_settings.docs_builder_tool)

    common_settings = settings.unit_settings("common-tools")
    assert_equal("common-tools", common_settings.name)
    assert_equal("unit", common_settings.type)
    ...
    assert_nil(common_settings.pre_builder_tool)
    assert_nil(common_settings.post_builder_tool)
