...
describe Toys::Release::RequestSpec do
  ...
  describe "#resolve_versions" do
    before do
      skip "Skipped integration test involving resolve_versions" unless ENV["TOYS_TEST_INTEGRATION"]
    end
    ...
    it "uses the VERSION constant with a bump when version_from_code is requested" do
      settings_text = <<~STRING
        components:
          - name: toys-release
      ...
      # This is just before the toys-release 0.8.0 release.
      request_spec.resolve_versions("7acf3481732a170133443743bf069989a28b1bd5", repository)
      resolved_components = request_spec.resolved_components
      assert_equal(1, resolved_components.size)

      component = resolved_components[0]
      assert_equal("toys-release", component.component_name)
      assert_equal(::Gem::Version.new("0.7.0.1"), component.version)
    end

    it "errors when version_from_code is requested with no bump but the VERSION already exists" do
      settings_text = <<~STRING
        components:
          - name: toys-release
            directory: toys-release
